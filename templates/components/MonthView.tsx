...
/**
 * MonthView — adapted for bahrawy-calendar package.
 *
 * All imports now come from 'bahrawy-calendar/compat' instead of relative paths.
 * Removed: useIsLaptopWidth (app-level hook).
 * Added: optional isLaptop prop (defaults to false, use your own media query hook).
 */

import React, { useMemo, memo, useCallback, useState } from 'react';
import type { CalendarEvent, EventInstance } from 'bahrawy-calendar';
import {
  useCalendarStore,
...
interface MonthViewProps {
  ...
  /**
   * Pass true when the viewport is laptop-width (< ~1200px).
   * Controls whether event pills render in compact (single-line) or full mode.
   * Hook into your own media query or use `window.matchMedia('(max-width: 1199px)')`.
   */
  isLaptop?: boolean;
}

const MonthView: React.FC<MonthViewProps> = ({ events, isLaptop = false }) => {
